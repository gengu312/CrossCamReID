...
双击或直接运行 `.\run_crosscam.bat` 会默认打开摄像头选择窗口，并优先勾选 `1,3` 两个外接摄像头。也可以运行 `.\select_cameras.bat` 或 `.\run_crosscam.bat -SelectCameras` 打开选择窗口。常用选项已改成中文，包括“使用已训练的铅笔/管子模型”“镜像画面”“交换左右”“显示轨迹线”“结束后自动检查运行结果”和“检查识别框是否频繁跳动”。RF-DETR 的模型大小、最低置信度和模型文件只在实验模式出现，一般保持默认即可。
