...
2. 先让目标在左侧摄像头画面里轻微移动，看到检测框后按 `r`，注册左侧当前最佳目标。
3. 如果目标先出现在右侧摄像头，看到检测框后按 `t`，注册右侧当前最佳目标。
...
5. 在摄像头 A 画面中按 `r`，注册当前最佳检测框为要追踪的目标。
...
- 给物体贴一个彩色贴纸。
- 减少背景运动。
- 按 `r/t` 前先轻微移动目标，让检测框尽量贴住物体。
- 如果自动检测框不准，再用 `m/n` 手动框选，尽量框紧一点，少包含背景和手。
- 调低目标模板阈值，例如：
