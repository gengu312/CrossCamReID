...
hybrid：YOLO 每帧主检，注册目标失配时按间隔调用 RF-DETR 补检
```

可以双击 `check_detector_backends.bat`，或运行下面的命令检查四种后端在当前电脑上是否真正就绪。摄像头选择窗口也会直接显示所选后端的依赖和项目权重状态：

```powershell
.\check_detector_backends.bat
```

推荐演示方式是先让目标在画面中产生检测框，再把当前最佳检测注册成具体目标，系统保存它的轻量视觉模板，之后在两个摄像头中持续寻找与它相似的候选。最终铁管场景应逐步从 `motion` 切到 `yolo`，再用自采集数据训练 `pipe` 类别模型；`rfdetr` 可单独与 YOLO 对比，也可在 `hybrid` 模式中作为按需补检，但当前试验权重还不能替换默认 YOLO。
...
- 支持 `--detector hybrid`，由 YOLO 主检，仅在已注册目标失配时调用 RF-DETR 补检。
- 对每个摄像头内的目标做简单跟踪。
- 为物体生成颜色、形状等轻量视觉指纹。
...
- 当前正式使用的 YOLO 模型已经训练完成。训练集由旧素材 151 张和筛选后的新素材 122 张组成，共 273 张，划分为 train=220、val=53；本机 CPU 共训练 12 轮。
- 本次没有把 262 张新照片无差别全部送入训练。重复图片、需要复核的自动标签和相似度较高的重复场景被隔离或暂不使用，避免错误标注和重复画面降低模型质量。
- 当前权重是 `runs_yolo/pipe_yolov8n_hybrid_0710_v2/weights/best.pt`，选择“YOLO 管子识别（推荐）”或启用 `-PipeMode` 时会自动加载。
- 增量模型在合并验证集上的 Precision=0.617、Recall=0.534、mAP50=0.567，已经优于原模型；它可以用于现阶段铅笔堆叠、拿取和移动演示，但还不能代表真实锈蚀铁管现场效果。
- RF-DETR 已使用同一批标注导出的 COCO 数据完成本机 CPU 单轮流程验证，并生成试验权重。该权重只证明训练、加载和补检流程可运行，不属于完整训练结果，也不建议代替当前 YOLO。
- 当前 YOLO 演示不依赖显卡服务器。后续若要完整训练 RF-DETR，或尝试更大 YOLO、OBB/实例分割和更多参数组合，建议使用 NVIDIA 显卡服务器。显卡主要提高训练和试验速度，使更多轮次、更大模型和更高分辨率成为可能；最终准确率仍取决于真实铁管数据和人工校正标签，显卡本身不会自动提高准确率。
...
程序分为“启动选择窗口”和“正式识别窗口”两步。选择窗口用于挑选 1 到 3 个摄像头、识别模式和画面设置；点击“启动识别”后，选择窗口会关闭，然后打开原来的摄像头识别、框选和追踪窗口。

```text
批处理入口
-> scripts/run_crosscam.ps1
-> src/camera_selector.py（启动选择窗口）
-> 点击“启动识别”
-> src/crosscam_mvp.py（正式识别窗口）
```
...
| 双击文件 | 用途 |
| --- | --- |
| `run_crosscam.bat` | 通用入口；在窗口中选择“YOLO 管子识别（推荐）”用于日常演示 |
| `run_pipe_hybrid.bat` | YOLO 主检、RF-DETR 漏检补检的实验入口 |
| `run_pipe_rfdetr.bat` | RF-DETR 单独检测的对比实验入口 |
| `select_cameras.bat` | 只打开通用摄像头选择入口 |

正常使用不需要直接运行 `src/camera_selector.py` 或 `src/crosscam_mvp.py`。摄像头预览只放在“摄像头”标签页中，一路显示一张小预览，两路并排，三路同一行；识别设置页不显示摄像头画面，不会挤占常用选项和底部启动按钮。
...
双击或直接运行 `.\run_crosscam.bat` 会默认打开摄像头选择窗口，并优先勾选 `1,3` 两个外接摄像头。也可以运行 `.\select_cameras.bat` 或 `.\run_crosscam.bat -SelectCameras` 打开选择窗口。常用选项已改成中文，包括“使用已训练的铅笔/管子模型”“镜像画面”“交换左右”“显示轨迹和方向箭头”“结束后自动检查运行结果”和“检查识别框是否频繁跳动”。RF-DETR 的模型大小、最低置信度和模型文件只在实验模式出现，一般保持默认即可。

若只想采集训练照片，在“测试与拍照”页选择拍摄内容和保存目录，勾选刚好 2 个摄像头后点击“采集训练照片”。如果没有探测到摄像头，可以勾选“摄像头打不开时进入演示画面”启动内置 demo；如果勾选“检查跨摄像头接力是否成功”，至少需要选择 2 个摄像头。通过命令行传入的读取方式、检测模式、模型路径和阈值参数会继续带入启动命令。若要跳过选择窗口，可以直接用 `-CameraIndexes "1,3"` 或 `-CameraIndexes "1,2,3"` 指定要打开的摄像头。
...
默认只显示当前锁定框和编号，不显示历史轨迹。若需要观察移动路径和当前方向，在启动界面勾选“显示轨迹和方向箭头”，或加 `-ShowTrails`。目标的最近路径会显示为轨迹线，轨迹末端箭头表示当前移动方向；目标静止或只有轻微抖动时不绘制方向箭头。
...
“可选择”表示代码入口已经接入。本机已经完成 RF-DETR Nano、CPU、1 轮的流程试验并生成权重，但这不是完整训练；当前正式识别仍以 YOLO 为主。若要判断 RF-DETR 是否真的优于 YOLO，需要在显卡服务器上使用同一训练集完成多轮训练，再用相同验证集和回放视频对比漏检、误检与锁定稳定性。
...
如果要运行 YOLO 与 RF-DETR 混合模式，两种依赖都要安装：
...
### 8. YOLO 主检 + RF-DETR 按需补检

两套模型并非只能二选一，但本机 CPU 不适合让它们对每一帧同时推理。`hybrid` 采用下面的顺序：

```text
YOLO 每帧检测
-> 已注册目标仍能匹配：直接使用 YOLO，不调用 RF-DETR
-> YOLO 没有匹配到目标：每隔 15 个处理帧最多调用一次 RF-DETR
-> 同一摄像头短时补检：候选还必须靠近上一次锁定位置
-> RF-DETR 候选仍需通过目标外观相似度和 ID 接管规则
-> 未通过：不恢复、不改目标 ID
```

多路摄像头顺序处理时会共享同一份 YOLO 和 RF-DETR 模型，避免为每路画面重复占用模型内存；每路摄像头的 ROI、编号和跟踪状态仍然独立。
...
也可以从摄像头选择窗口选择“YOLO + RF-DETR 补检（实验）”，并调整“漏检后每隔几帧尝试一次”。数值越小，丢失后尝试越频繁，但 CPU 越慢；本机默认 `15`，有 NVIDIA 显卡后可从 `5` 开始测试。

本机用默认间隔回放遮挡录像 500 帧：RF-DETR 调用 14 次、产生 164 个候选、恢复 G001 1 次；该次候选相似度为 0.777，距上次锁定位置 19.8 像素。组合路径已经实际触发，但当前注册框包含多根相邻铅笔，暂时只能确认软件锁定恢复，不能据此证明物理上一定是同一根。日常演示仍使用 YOLO，混合模式只用于实验。
